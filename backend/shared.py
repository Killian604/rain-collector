...
def generate_new_caller_prompt():
    return random.choices(
        ...
        weights=[100 / len(default_callers) for _ in default_callers] + [10 / len(mildlyrareprompts) for _ in range(len(mildlyrareprompts))] + [1 for _ in range( len(rare_prompts))],
    )[0]
